...
public class XmlUtil {
    ...
    public static Map<String, List<MultiLanguageBean>> paresXmlMultiLanguage(File rootDir) {
        Map<String, List<MultiLanguageBean>> datas = new HashMap<>();
        try {
            //C:\WorkSpace\Project\Android\Zaful\
            //includesPattern= **/app/src/
            ...
            if (hasElements) {
                for (int index = 0; index < length; index++) {
                    ...
                    if (nNode.getNodeType() == Node.ELEMENT_NODE) {
                        ...
                        if (translatable == null || !hasTranslatable || Utils.toBoolean(translatable)) {
