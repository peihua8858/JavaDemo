...
public class ExcelUtil {
    ...
    public static Map<String, List<ElementBean>> parseExcelForMap2(Sheet sheet) {
        ...
        for (Row row : sheet) {
            ...
            for (Cell cell : row) {
                if (valueKey == null) {
                    valueKey = Utils.toString(getCellValue(cell));
                    continue;
                }
                //第一行当前列
                final Cell firstRowCurCell = firstRow.getCell(cell.getColumnIndex());
                if (firstRowCurCell != null) {
                    final String languageCode = languageCode(Utils.toString(getCellValue(firstRowCurCell)));
                    final String cellValue = Utils.toString(cellValue(row, cell));
                    if (cellValue != null && languageCode != null) {
